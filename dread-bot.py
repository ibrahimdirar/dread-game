        ...
        if ctx.author.name in player_list:
            for i in range(0, pulls):
                    ...
                    if pull > big_frightening_number:
                        await ctx.send('The tower has fallen!')
                        await ctx.send(ctx.author.name + ' has died!')
                        death_count = death_count + 1
                        player_list.pop(ctx.author.name)
                        if not player_list:
                            await ctx.send('All characters are dead!')
                        else:
                            await ctx.send('Generating a new tower!')
                            big_frightening_number = 100
                            await ctx.send('As ' + str(death_count) + ' players have died, ' + str(death_count*3) + ' pulls are required.')
                            ...
                            for i in range(0,death_count*3):
                                    ...
                                    await ctx.send('Player ' + drawing_player + ' must draw one')
                        break
                    else:
